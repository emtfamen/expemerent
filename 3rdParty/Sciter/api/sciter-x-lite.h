 ...
 * \return \b SCLRESULT.
 **/
EXTERN_C  SCLRESULT SCAPI SciliteLoadFromMemory(SCILITE hLite, LPCWSTR baseURI, LPCBYTE dataptr, UINT datasize);
...
EXTERN_C  SCLRESULT SCAPI SciliteSetDataReadyAsync(SCILITE hLite, LPCWSTR uri, LPCBYTE data, DWORD dataSize, UINT type);
...
typedef UINT CALLBACK SCILITE_CALLBACK(SCILITE hLite, LPNMHDR hdr);
...
class  Scilite
{
  ...
  static UINT CALLBACK SciliteCB(SCILITE hLite, LPNMHDR hdr)
  {
    Scilite *pex = 0;
    SciliteGetTag(hLite, (void**)&pex);
    assert(pex);
...
public:
  ...
  {
    SCLRESULT hr = SciliteLoadFromMemory(hLite, baseURI, dataptr, datasize);
    assert(hr == SCL_OK);
    return hr == SCL_OK;
  }
  ...
  {
    SCLRESULT hr = SciliteSetDataReadyAsync(hLite, url, data, dataSize,dataType);
    assert(hr == SCL_OK);
    return hr == SCL_OK;
  }
