...
/**
 * ValueFloatData - retreive FLOAT (double) data of T_FLOAT and T_LENGTH values.
 */
EXTERN_C UINT VALAPI ValueFloatData( const VALUE* pval, FLOAT* pData );

/**
 * ValueFloatDataSet - sets FLOAT data of T_FLOAT and T_LENGTH values.
 */
EXTERN_C UINT VALAPI ValueFloatDataSet( VALUE* pval, FLOAT data, UINT type, UINT units );
  ...
  namespace json
  {
    ...
    public:
     ...
     ~value()                 { ValueClear(this); }
      ...
      static value currency( INT64 v )  { value t; ValueInt64DataSet(&t, v, T_CURRENCY, 0); }
      static value date( INT64 v )      { value t; ValueInt64DataSet(&t, v, T_DATE, 0); }
#ifdef WIN32
      static value date( FILETIME ft )  { value t; ValueInt64DataSet(&t, *((INT64*)&ft), T_DATE, 0); } 
#endif
